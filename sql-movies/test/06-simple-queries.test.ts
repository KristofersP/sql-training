...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) as count FROM movies WHERE movies.budget > 100000000 AND movies.release_date > 2009`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT original_title, budget, revenue FROM movies WHERE movies.release_date > 2009
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) as count from movies WHERE movies.homepage LIKE 'https%'`;
      const result = await db.selectSingleRow(query);
